...
app.get('/api/colaboradores/:id', async (req, res) => {
  try {
    ...
    res.json(await mapColaborador(rows[0]));
  } catch (error) {
...
app.get('/api/habilidades', async (req, res) => {
  try {
    const [rows] = await pool.query('SELECT * FROM habilidades ORDER BY sigla');
    res.json(rows.map(mapHabilidade));
  } catch (error) {
...
app.post('/api/habilidades', async (req, res) => {
  ...
  console.log('[API POST /habilidades] Recebido:', { sigla, descricao, dominio, subcategoria });
  
  if (!sigla || !descricao || !dominio || !subcategoria) {
    console.log('[API POST /habilidades] Campos faltando');
    return res.status(400).json({
      error: 'Campos obrigatórios faltando',
      code: 'MISSING_FIELDS',
      missing: [
  ...
  const siglaRegex = /^[A-Z0-9]{6}-\d{5}$/;
  if (!siglaRegex.test(sigla)) {
    return res.status(400).json({
      error: 'Formato de sigla inválido. Use: ABC123-12345 (6 caracteres alfanuméricos maiúsculos, hífen, 5 dígitos)',
      code: 'INVALID_FORMAT'
    });
  }
  ...
  try {
    const id = uuidv4();
    console.log('[API POST /habilidades] Inserindo no banco...', { id, sigla: sigla.toUpperCase(), descricao, dominio, subcategoria });
    ...
    console.log('[API POST /habilidades] Inserido com sucesso, buscando registro...');
    const [created] = await pool.query('SELECT * FROM habilidades WHERE id = ?', [id]);
    console.log('[API POST /habilidades] Registro recuperado:', created[0]);
    
    const resultado = mapHabilidade(created[0]);
    console.log('[API POST /habilidades] Enviando resposta:', resultado);
    res.status(201).json(resultado);
  } catch (error) {
    console.error('[API POST /habilidades] ERRO:', error);
    console.error('[API POST /habilidades] Código do erro:', error.code);
    console.error('[API POST /habilidades] Mensagem:', error.message);
    console.error('[API POST /habilidades] SQL State:', error.sqlState);
    console.error('[API POST /habilidades] SQL:', error.sql);
    
    if (error.code === 'ER_DUP_ENTRY') {
      return res.status(409).json({ error: 'Habilidade já existe', code: 'DUPLICATE' });
    }
    if (error.code === 'ER_NO_SUCH_TABLE') {
      return res.status(500).json({ error: 'Tabela habilidades não existe. Execute a migração primeiro.', code: 'TABLE_NOT_FOUND' });
    }
    if (error.code === 'ER_BAD_FIELD_ERROR') {
      return res.status(500).json({ error: 'Estrutura da tabela incorreta. Execute a migração.', code: 'WRONG_SCHEMA' });
    }
    res.status(500).json({ error: 'Erro ao salvar dados: ' + error.message, code: 'DATABASE_ERROR' });
  }
});
...
app.put('/api/habilidades/:id', async (req, res) => {
  ...
  // Validar formato da sigla se fornecida
  if (sigla) {
    const siglaRegex = /^[A-Z0-9]{6}-\d{5}$/;
    if (!siglaRegex.test(sigla)) {
      return res.status(400).json({
  ...
  try {
    const [existing] = await pool.query('SELECT * FROM habilidades WHERE id = ?', [req.params.id]);
    if (existing.length === 0) {
      return res.status(404).json({ error: 'Habilidade não encontrada', code: 'NOT_FOUND' });
    }
    ...
    const [updated] = await pool.query('SELECT * FROM habilidades WHERE id = ?', [req.params.id]);
    res.json(mapHabilidade(updated[0]));
  } catch (error) {
...
app.delete('/api/habilidades/:id', async (req, res) => {
  try {
    // Verificar se a habilidade está sendo usada em colaborador_habilidades
    const [dependencies] = await pool.query(
      'SELECT COUNT(*) as count FROM colaborador_habilidades WHERE habilidade_id = ?',
      [req.params.id]
    );

    if (dependencies[0].count > 0) {
      return res.status(400).json({
        error: `Não é possível excluir esta habilidade pois ela está sendo utilizada por ${dependencies[0].count} colaborador(es)`,
...
app.post('/api/capacidades-negocio', async (req, res) => {
  ...
  try {
    ...
    res.status(201).json(mapCapacidadeNegocio(created[0]));
  } catch (error) {
...
app.post('/api/tecnologias', async (req, res) => {
  ...
  try {
    const id = uuidv4();
    // Gerar sigla: primeiro remove espaços, depois pega substring
    const tecnologiaSigla = sigla || nome.toUpperCase().replace(/\s/g, '-').substring(0, 20);
    ...
    await pool.query(
      `INSERT INTO tecnologias 
       (id, sigla, nome, versao_release, categoria, status, 
        ...
        nivel_suporte_interno, documentacao_oficial, repositorio_interno,
        ambiente_dev, ambiente_qa, ambiente_prod, 
      ...
      [
        ...
        categoria || null, 
        status || null,
        fornecedorFabricante || null,
        tipoLicenciamento || null,
        maturidadeInterna || null,
        nivelSuporteInterno || null,
        documentacaoOficial || null,
        repositorioInterno || null,
        ambientes?.dev ? 1 : 0,
        ambientes?.qa ? 1 : 0,
        ambientes?.prod ? 1 : 0,
        ambientes?.cloud ? 1 : 0,
        ambientes?.onPremise ? 1 : 0
      ]
    );
    ...
    if (error.code === 'ER_DUP_ENTRY') {
      return res.status(409).json({ error: 'Tecnologia já existe', code: 'DUPLICATE' });
    }
    res.status(500).json({ error: 'Erro ao salvar dados', code: 'DATABASE_ERROR' });
  }
});
...
app.put('/api/tecnologias/:id', async (req, res) => {
  ...
  try {
    const [existing] = await pool.query('SELECT * FROM tecnologias WHERE id = ?', [req.params.id]);
    if (existing.length === 0) {
      return res.status(404).json({ error: 'Tecnologia não encontrada', code: 'NOT_FOUND' });
    }
    ...
    await pool.query(
      ...
      [
        sigla !== undefined ? sigla : existing[0].sigla,
        nome || existing[0].nome,
        versaoRelease !== undefined ? versaoRelease : existing[0].versao_release,
        categoria !== undefined ? categoria : existing[0].categoria,
        status !== undefined ? status : existing[0].status,
        fornecedorFabricante !== undefined ? fornecedorFabricante : existing[0].fornecedor_fabricante,
        tipoLicenciamento !== undefined ? tipoLicenciamento : existing[0].tipo_licenciamento,
        maturidadeInterna !== undefined ? maturidadeInterna : existing[0].maturidade_interna,
        nivelSuporteInterno !== undefined ? nivelSuporteInterno : existing[0].nivel_suporte_interno,
        documentacaoOficial !== undefined ? documentacaoOficial : existing[0].documentacao_oficial,
        repositorioInterno !== undefined ? repositorioInterno : existing[0].repositorio_interno,
        ambientes?.dev !== undefined ? (ambientes.dev ? 1 : 0) : existing[0].ambiente_dev,
        ambientes?.qa !== undefined ? (ambientes.qa ? 1 : 0) : existing[0].ambiente_qa,
        ambientes?.prod !== undefined ? (ambientes.prod ? 1 : 0) : existing[0].ambiente_prod,
        ambientes?.cloud !== undefined ? (ambientes.cloud ? 1 : 0) : existing[0].ambiente_cloud,
        ambientes?.onPremise !== undefined ? (ambientes.onPremise ? 1 : 0) : existing[0].ambiente_on_premise,
        req.params.id
      ]
    );
...
app.post('/api/aplicacoes', async (req, res) => {
  ...
  try {
    if (!sigla || !descricao || !urlDocumentacao || !faseCicloVida || !criticidadeNegocio) {
      return res.status(400).json({
        error: 'Campos obrigatórios faltando',
        code: 'MISSING_FIELDS',
        missing: [
    ...
    await pool.query(
      ...
      [
        id, sigla, descricao, urlDocumentacao, tipoAplicacao || null, faseCicloVida, criticidadeNegocio,
    ...
    // Salvar tecnologias
    console.log('[API POST /aplicacoes] Iniciando salvamento de tecnologias...');
    console.log('[API POST /aplicacoes] tecnologias variável:', tecnologias);
    console.log('[API POST /aplicacoes] É array?', Array.isArray(tecnologias));
    console.log('[API POST /aplicacoes] Length:', tecnologias?.length);
    
    if (tecnologias && Array.isArray(tecnologias)) {
      console.log('[API POST /aplicacoes] Processando', tecnologias.length, 'tecnologias');
      for (const tec of tecnologias) {
        console.log('[API POST /aplicacoes] Salvando tecnologia:', tec);
        
        // Converter datas para formato MySQL (YYYY-MM-DD)
        const dataInicio = tec.dataInicio 
          ? (typeof tec.dataInicio === 'string' ? tec.dataInicio.split('T')[0] : tec.dataInicio)
          : new Date().toISOString().split('T')[0];
        
        const dataTermino = tec.dataTermino 
          ? (typeof tec.dataTermino === 'string' ? tec.dataTermino.split('T')[0] : tec.dataTermino)
          : null;
        ...
        console.log('[API POST /aplicacoes] Valores do INSERT:', values);
        
        await pool.query(
          'INSERT INTO aplicacao_tecnologias (id, aplicacao_id, tecnologia_id, data_inicio, data_termino, status) VALUES (?, ?, ?, ?, ?, ?)',
          values
        );
        console.log('[API POST /aplicacoes] ✓ Tecnologia salva');
      }
      console.log('[API POST /aplicacoes] ✓ Todas as tecnologias salvas');
    } else {
      console.log('[API POST /aplicacoes] ⚠️ Nenhuma tecnologia para salvar (undefined ou não é array)');
    }
...
app.put('/api/aplicacoes/:id', async (req, res) => {
  ...
  try {
    ...
    console.log('[API PUT /aplicacoes] Iniciando atualização de tecnologias...');
    console.log('[API PUT /aplicacoes] tecnologias variável:', tecnologias);
    console.log('[API PUT /aplicacoes] É undefined?', tecnologias === undefined);
    console.log('[API PUT /aplicacoes] É array?', Array.isArray(tecnologias));
    console.log('[API PUT /aplicacoes] Length:', tecnologias?.length);
    
    if (tecnologias !== undefined) {
      console.log('[API PUT /aplicacoes] Deletando tecnologias antigas...');
      await pool.query('DELETE FROM aplicacao_tecnologias WHERE aplicacao_id = ?', [req.params.id]);
      console.log('[API PUT /aplicacoes] ✓ Tecnologias antigas deletadas');
      
      if (Array.isArray(tecnologias)) {
        console.log('[API PUT /aplicacoes] Processando', tecnologias.length, 'tecnologias');
        for (const tec of tecnologias) {
          console.log('[API PUT /aplicacoes] Salvando tecnologia:', tec);
          console.log('[API PUT /aplicacoes] tec.dataInicio ANTES da conversão:', tec.dataInicio);
          console.log('[API PUT /aplicacoes] typeof tec.dataInicio:', typeof tec.dataInicio);
          
          // Converter datas para formato MySQL (YYYY-MM-DD)
          let dataInicio = tec.dataInicio;
          if (dataInicio) {
            if (typeof dataInicio === 'string') {
              // Se contém 'T', é ISO format, pegar só a parte da data
          ...
          console.log('[API PUT /aplicacoes] dataInicio DEPOIS da conversão:', dataInicio);
          console.log('[API PUT /aplicacoes] dataTermino DEPOIS da conversão:', dataTermino);
          ...
          console.log('[API PUT /aplicacoes] Valores do INSERT:', values);
          
          await pool.query(
            'INSERT INTO aplicacao_tecnologias (id, aplicacao_id, tecnologia_id, data_inicio, data_termino, status) VALUES (?, ?, ?, ?, ?, ?)',
            values
          );
          console.log('[API PUT /aplicacoes] ✓ Tecnologia salva');
        }
        console.log('[API PUT /aplicacoes] ✓ Todas as tecnologias salvas');
      } else {
        console.log('[API PUT /aplicacoes] ℹ️ Array de tecnologias vazio ou não é array');
      }
    } else {
      console.log('[API PUT /aplicacoes] ℹ️ tecnologias é undefined - não atualizar');
    }
    ...
    if (integracoes !== undefined) {
      await pool.query('DELETE FROM aplicacao_integracoes WHERE aplicacao_id = ?', [req.params.id]);
...
app.delete('/api/aplicacoes/:id', async (req, res) => {
  try {
    const [result] = await pool.query('DELETE FROM aplicacoes WHERE id = ?', [req.params.id]);
    if (result.affectedRows === 0) {
      return res.status(404).json({ error: 'Aplicação não encontrada', code: 'NOT_FOUND' });
    }
    res.status(204).send();
...
app.get('/api/integracoes', async (req, res) => {
  try {
    const [rows] = await pool.query('SELECT id, sigla, nome, estilo_integracao, padrao_caso_uso, integracao_tecnologica, especificacao_filename, especificacao_mimetype, created_at, updated_at FROM integracoes ORDER BY sigla');
    res.json(rows.map(row => ({
      id: row.id,
      sigla: row.sigla,
      nome: row.nome,
...
app.get('/api/integracoes/:id', async (req, res) => {
  try {
    const [rows] = await pool.query('SELECT id, sigla, nome, estilo_integracao, padrao_caso_uso, integracao_tecnologica, especificacao_filename, especificacao_mimetype, created_at, updated_at FROM integracoes WHERE id = ?', [req.params.id]);
    if (rows.length === 0) {
...
app.post('/api/integracoes', upload.single('especificacao'), async (req, res) => {
  const { sigla, nome, estiloIntegracao, padraoCasoUso, integracaoTecnologica } = req.body;
  ...
  if (sigla.length > 20) {
    return res.status(400).json({ error: 'Sigla deve ter no máximo 20 caracteres', code: 'INVALID_LENGTH' });
  }
  
  if (nome.length > 30) {
    return res.status(400).json({ error: 'Nome deve ter no máximo 30 caracteres', code: 'INVALID_LENGTH' });
  }
  ...
  try {
    ...
    if (req.file) {
      await pool.query(
        'INSERT INTO integracoes (id, sigla, nome, estilo_integracao, padrao_caso_uso, integracao_tecnologica, especificacao_filename, especificacao_mimetype, especificacao_data) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
        [id, sigla, nome, estiloIntegracao || null, padraoCasoUso || null, integracaoTecnologica || null, req.file.originalname, req.file.mimetype, req.file.buffer]
      );
    } else {
      await pool.query(
        'INSERT INTO integracoes (id, sigla, nome, estilo_integracao, padrao_caso_uso, integracao_tecnologica) VALUES (?, ?, ?, ?, ?, ?)',
        [id, sigla, nome, estiloIntegracao || null, padraoCasoUso || null, integracaoTecnologica || null]
      );
    }
    
    const [created] = await pool.query('SELECT id, sigla, nome, estilo_integracao, padrao_caso_uso, integracao_tecnologica, especificacao_filename, especificacao_mimetype, created_at, updated_at FROM integracoes WHERE id = ?', [id]);
    res.status(201).json({
      id: created[0].id,
...
app.put('/api/integracoes/:id', upload.single('especificacao'), async (req, res) => {
  ...
  try {
    ...
    console.log('[PUT /api/integracoes/:id] Valores existentes:', {
      estilo_integracao: existing[0].estilo_integracao,
      padrao_caso_uso: existing[0].padrao_caso_uso,
      integracao_tecnologica: existing[0].integracao_tecnologica
    });
    
    if (req.file) {
      await pool.query(
        'UPDATE integracoes SET sigla = ?, nome = ?, estilo_integracao = ?, padrao_caso_uso = ?, integracao_tecnologica = ?, especificacao_filename = ?, especificacao_mimetype = ?, especificacao_data = ? WHERE id = ?',
        [
      ...
      await pool.query(
        'UPDATE integracoes SET sigla = ?, nome = ?, estilo_integracao = ?, padrao_caso_uso = ?, integracao_tecnologica = ? WHERE id = ?',
        [
          sigla || existing[0].sigla, 
          nome || existing[0].nome, 
          estiloIntegracao !== undefined ? estiloIntegracao : existing[0].estilo_integracao,
    ...
    const [updated] = await pool.query('SELECT id, sigla, nome, estilo_integracao, padrao_caso_uso, integracao_tecnologica, especificacao_filename, especificacao_mimetype, created_at, updated_at FROM integracoes WHERE id = ?', [req.params.id]);
    res.json({
      id: updated[0].id,
...
// User-to-Cloud
app.get('/api/user-to-cloud', async (req, res) => {
  try {
    const integracaoId = req.query.integracaoId;
    const query = integracaoId 
      ? 'SELECT * FROM user_to_cloud WHERE integracao_id = ? ORDER BY created_at DESC'
      : 'SELECT * FROM user_to_cloud ORDER BY created_at DESC';
    const params = integracaoId ? [integracaoId] : [];
    ...
    console.error('Erro ao listar user-to-cloud:', error);
    res.status(500).json({ error: 'Erro ao buscar dados', code: 'DATABASE_ERROR' });
  }
});

app.post('/api/user-to-cloud', async (req, res) => {
  const { integracaoId, tipoDispositivo, nomeDispositivo, comunicacaoId, tipoAutenticacao, periodicidade, frequenciaUso } = req.body;
  
  if (!integracaoId || !tipoDispositivo || !nomeDispositivo || !comunicacaoId || !tipoAutenticacao || !periodicidade || !frequenciaUso) {
  ...
  try {
    const id = uuidv4();
    await pool.query(
      'INSERT INTO user_to_cloud (id, integracao_id, tipo_dispositivo, nome_dispositivo, comunicacao_id, tipo_autenticacao, periodicidade, frequencia_uso) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
      [id, integracaoId, tipoDispositivo, nomeDispositivo, comunicacaoId, tipoAutenticacao, periodicidade, frequenciaUso]
    );
    const [created] = await pool.query('SELECT * FROM user_to_cloud WHERE id = ?', [id]);
    res.status(201).json({
      id: created[0].id,
      integracaoId: created[0].integracao_id,
    ...
    console.error('Erro ao criar user-to-cloud:', error);
    res.status(500).json({ error: 'Erro ao salvar dados', code: 'DATABASE_ERROR' });
  }
});

app.put('/api/user-to-cloud/:id', async (req, res) => {
  const { tipoDispositivo, nomeDispositivo, comunicacaoId, tipoAutenticacao, periodicidade, frequenciaUso } = req.body;
  
  try {
    const [existing] = await pool.query('SELECT * FROM user_to_cloud WHERE id = ?', [req.params.id]);
    if (existing.length === 0) {
      return res.status(404).json({ error: 'Registro não encontrado', code: 'NOT_FOUND' });
    }
    
    await pool.query(
      'UPDATE user_to_cloud SET tipo_dispositivo = ?, nome_dispositivo = ?, comunicacao_id = ?, tipo_autenticacao = ?, periodicidade = ?, frequencia_uso = ? WHERE id = ?',
      [tipoDispositivo || existing[0].tipo_dispositivo, nomeDispositivo || existing[0].nome_dispositivo, comunicacaoId || existing[0].comunicacao_id, tipoAutenticacao || existing[0].tipo_autenticacao, periodicidade || existing[0].periodicidade, frequenciaUso || existing[0].frequencia_uso, req.params.id]
    );
    
    const [updated] = await pool.query('SELECT * FROM user_to_cloud WHERE id = ?', [req.params.id]);
    res.json({
      id: updated[0].id,
      integracaoId: updated[0].integracao_id,
    ...
    console.error('Erro ao atualizar user-to-cloud:', error);
    res.status(500).json({ error: 'Erro ao atualizar dados', code: 'DATABASE_ERROR' });
  }
});

app.delete('/api/user-to-cloud/:id', async (req, res) => {
  try {
    const [result] = await pool.query('DELETE FROM user_to_cloud WHERE id = ?', [req.params.id]);
    if (result.affectedRows === 0) {
      return res.status(404).json({ error: 'Registro não encontrado', code: 'NOT_FOUND' });
    }
    res.status(204).send();
  } catch (error) {
    console.error('Erro ao excluir user-to-cloud:', error);
    res.status(500).json({ error: 'Erro ao excluir dados', code: 'DATABASE_ERROR' });
  }
});

// User-to-OnPremise (código similar, adaptado)
app.get('/api/user-to-onpremise', async (req, res) => {
  try {
    const integracaoId = req.query.integracaoId;
    const query = integracaoId 
      ? 'SELECT * FROM user_to_onpremise WHERE integracao_id = ? ORDER BY created_at DESC'
      : 'SELECT * FROM user_to_onpremise ORDER BY created_at DESC';
    const params = integracaoId ? [integracaoId] : [];
    const [rows] = await pool.query(query, params);
    res.json(rows.map(row => ({
      id: row.id,
    ...
    console.error('Erro ao listar user-to-onpremise:', error);
    res.status(500).json({ error: 'Erro ao buscar dados', code: 'DATABASE_ERROR' });
  }
});

app.post('/api/user-to-onpremise', async (req, res) => {
  const { integracaoId, tipoDispositivo, nomeDispositivo, comunicacaoId, tipoAutenticacao, periodicidade, frequenciaUso } = req.body;
  ...
  try {
    const id = uuidv4();
    await pool.query(
      'INSERT INTO user_to_onpremise (id, integracao_id, tipo_dispositivo, nome_dispositivo, comunicacao_id, tipo_autenticacao, periodicidade, frequencia_uso) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
      [id, integracaoId, tipoDispositivo, nomeDispositivo, comunicacaoId, tipoAutenticacao, periodicidade, frequenciaUso]
    );
    const [created] = await pool.query('SELECT * FROM user_to_onpremise WHERE id = ?', [id]);
    res.status(201).json({
      id: created[0].id,
      integracaoId: created[0].integracao_id,
      tipoDispositivo: created[0].tipo_dispositivo,
    ...
    console.error('Erro ao criar user-to-onpremise:', error);
    res.status(500).json({ error: 'Erro ao salvar dados', code: 'DATABASE_ERROR' });
  }
});

app.put('/api/user-to-onpremise/:id', async (req, res) => {
  const { tipoDispositivo, nomeDispositivo, comunicacaoId, tipoAutenticacao, periodicidade, frequenciaUso } = req.body;
  
  try {
    const [existing] = await pool.query('SELECT * FROM user_to_onpremise WHERE id = ?', [req.params.id]);
    if (existing.length === 0) {
      return res.status(404).json({ error: 'Registro não encontrado', code: 'NOT_FOUND' });
    }
    
    await pool.query(
      'UPDATE user_to_onpremise SET tipo_dispositivo = ?, nome_dispositivo = ?, comunicacao_id = ?, tipo_autenticacao = ?, periodicidade = ?, frequencia_uso = ? WHERE id = ?',
      [tipoDispositivo || existing[0].tipo_dispositivo, nomeDispositivo || existing[0].nome_dispositivo, comunicacaoId || existing[0].comunicacao_id, tipoAutenticacao || existing[0].tipo_autenticacao, periodicidade || existing[0].periodicidade, frequenciaUso || existing[0].frequencia_uso, req.params.id]
    );
    
    const [updated] = await pool.query('SELECT * FROM user_to_onpremise WHERE id = ?', [req.params.id]);
    res.json({
      id: updated[0].id,
      integracaoId: updated[0].integracao_id,
      tipoDispositivo: updated[0].tipo_dispositivo,
    ...
    console.error('Erro ao atualizar user-to-onpremise:', error);
    res.status(500).json({ error: 'Erro ao atualizar dados', code: 'DATABASE_ERROR' });
  }
});

app.delete('/api/user-to-onpremise/:id', async (req, res) => {
  try {
    const [result] = await pool.query('DELETE FROM user_to_onpremise WHERE id = ?', [req.params.id]);
    if (result.affectedRows === 0) {
      return res.status(404).json({ error: 'Registro não encontrado', code: 'NOT_FOUND' });
    }
    res.status(204).send();
  } catch (error) {
    console.error('Erro ao excluir user-to-onpremise:', error);
    res.status(500).json({ error: 'Erro ao excluir dados', code: 'DATABASE_ERROR' });
  }
});

// Cloud-to-Cloud
app.get('/api/cloud-to-cloud', async (req, res) => {
  try {
    const integracaoId = req.query.integracaoId;
    const query = integracaoId 
      ? 'SELECT * FROM cloud_to_cloud WHERE integracao_id = ? ORDER BY created_at DESC'
      : 'SELECT * FROM cloud_to_cloud ORDER BY created_at DESC';
    const params = integracaoId ? [integracaoId] : [];
    const [rows] = await pool.query(query, params);
    res.json(rows.map(row => ({
      id: row.id,
      ...
      tipoIntegracaoId: row.tipo_integracao_id,
      tipoAutenticacao: row.tipo_autenticacao,
      periodicidade: row.periodicidade,
      frequenciaUso: row.frequencia_uso,
      createdAt: row.created_at,
    ...
    console.error('Erro ao listar cloud-to-cloud:', error);
    res.status(500).json({ error: 'Erro ao buscar dados', code: 'DATABASE_ERROR' });
  }
});

app.post('/api/cloud-to-cloud', async (req, res) => {
  const { integracaoId, aplicacaoOrigemId, aplicacaoDestinoId, tipoIntegracaoId, tipoAutenticacao, periodicidade, frequenciaUso } = req.body;
  
  if (!integracaoId || !aplicacaoOrigemId || !aplicacaoDestinoId || !tipoIntegracaoId || !tipoAutenticacao || !periodicidade || !frequenciaUso) {
    return res.status(400).json({ error: 'Todos os campos são obrigatórios', code: 'MISSING_FIELDS' });
  }
  ...
  try {
    const id = uuidv4();
    await pool.query(
      'INSERT INTO cloud_to_cloud (id, integracao_id, aplicacao_origem_id, aplicacao_destino_id, tipo_integracao_id, tipo_autenticacao, periodicidade, frequencia_uso) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
      [id, integracaoId, aplicacaoOrigemId, aplicacaoDestinoId, tipoIntegracaoId, tipoAutenticacao, periodicidade, frequenciaUso]
    );
    const [created] = await pool.query('SELECT * FROM cloud_to_cloud WHERE id = ?', [id]);
    res.status(201).json({
      id: created[0].id,
      integracaoId: created[0].integracao_id,
      aplicacaoOrigemId: created[0].aplicacao_origem_id,
      aplicacaoDestinoId: created[0].aplicacao_destino_id,
      tipoIntegracaoId: created[0].tipo_integracao_id,
      tipoAutenticacao: created[0].tipo_autenticacao,
      periodicidade: created[0].periodicidade,
      frequenciaUso: created[0].frequencia_uso,
      createdAt: created[0].created_at,
    ...
    console.error('Erro ao criar cloud-to-cloud:', error);
    res.status(500).json({ error: 'Erro ao salvar dados', code: 'DATABASE_ERROR' });
  }
});

app.put('/api/cloud-to-cloud/:id', async (req, res) => {
  const { aplicacaoOrigemId, aplicacaoDestinoId, tipoIntegracaoId, tipoAutenticacao, periodicidade, frequenciaUso } = req.body;
  
  try {
    const [existing] = await pool.query('SELECT * FROM cloud_to_cloud WHERE id = ?', [req.params.id]);
    if (existing.length === 0) {
      return res.status(404).json({ error: 'Registro não encontrado', code: 'NOT_FOUND' });
    }
    
    await pool.query(
      'UPDATE cloud_to_cloud SET aplicacao_origem_id = ?, aplicacao_destino_id = ?, tipo_integracao_id = ?, tipo_autenticacao = ?, periodicidade = ?, frequencia_uso = ? WHERE id = ?',
      [aplicacaoOrigemId || existing[0].aplicacao_origem_id, aplicacaoDestinoId || existing[0].aplicacao_destino_id, tipoIntegracaoId || existing[0].tipo_integracao_id, tipoAutenticacao || existing[0].tipo_autenticacao, periodicidade || existing[0].periodicidade, frequenciaUso || existing[0].frequencia_uso, req.params.id]
    );
    
    const [updated] = await pool.query('SELECT * FROM cloud_to_cloud WHERE id = ?', [req.params.id]);
    res.json({
      id: updated[0].id,
      integracaoId: updated[0].integracao_id,
      aplicacaoOrigemId: updated[0].aplicacao_origem_id,
      aplicacaoDestinoId: updated[0].aplicacao_destino_id,
      tipoIntegracaoId: updated[0].tipo_integracao_id,
      tipoAutenticacao: updated[0].tipo_autenticacao,
      periodicidade: updated[0].periodicidade,
      frequenciaUso: updated[0].frequencia_uso,
      createdAt: updated[0].created_at,
    ...
    console.error('Erro ao atualizar cloud-to-cloud:', error);
    res.status(500).json({ error: 'Erro ao atualizar dados', code: 'DATABASE_ERROR' });
  }
});

app.delete('/api/cloud-to-cloud/:id', async (req, res) => {
  try {
    const [result] = await pool.query('DELETE FROM cloud_to_cloud WHERE id = ?', [req.params.id]);
    if (result.affectedRows === 0) {
      return res.status(404).json({ error: 'Registro não encontrado', code: 'NOT_FOUND' });
    }
    res.status(204).send();
  } catch (error) {
    console.error('Erro ao excluir cloud-to-cloud:', error);
    res.status(500).json({ error: 'Erro ao excluir dados', code: 'DATABASE_ERROR' });
  }
});

// OnPremise-to-Cloud
app.get('/api/onpremise-to-cloud', async (req, res) => {
  try {
    const integracaoId = req.query.integracaoId;
    const query = integracaoId 
      ? 'SELECT * FROM onpremise_to_cloud WHERE integracao_id = ? ORDER BY created_at DESC'
      : 'SELECT * FROM onpremise_to_cloud ORDER BY created_at DESC';
    const params = integracaoId ? [integracaoId] : [];
    const [rows] = await pool.query(query, params);
    res.json(rows.map(row => ({
      id: row.id,
    ...
    console.error('Erro ao listar onpremise-to-cloud:', error);
    res.status(500).json({ error: 'Erro ao buscar dados', code: 'DATABASE_ERROR' });
  }
});

app.post('/api/onpremise-to-cloud', async (req, res) => {
  const { integracaoId, aplicacaoOrigemId, aplicacaoDestinoId, tipoIntegracaoId, tipoAutenticacao, periodicidade, frequenciaUso } = req.body;
  ...
  try {
    const id = uuidv4();
    await pool.query(
      'INSERT INTO onpremise_to_cloud (id, integracao_id, aplicacao_origem_id, aplicacao_destino_id, tipo_integracao_id, tipo_autenticacao, periodicidade, frequencia_uso) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
      [id, integracaoId, aplicacaoOrigemId, aplicacaoDestinoId, tipoIntegracaoId, tipoAutenticacao, periodicidade, frequenciaUso]
    );
    const [created] = await pool.query('SELECT * FROM onpremise_to_cloud WHERE id = ?', [id]);
    res.status(201).json({
      id: created[0].id,
      integracaoId: created[0].integracao_id,
      aplicacaoOrigemId: created[0].aplicacao_origem_id,
    ...
    console.error('Erro ao criar onpremise-to-cloud:', error);
    res.status(500).json({ error: 'Erro ao salvar dados', code: 'DATABASE_ERROR' });
  }
});

app.put('/api/onpremise-to-cloud/:id', async (req, res) => {
  const { aplicacaoOrigemId, aplicacaoDestinoId, tipoIntegracaoId, tipoAutenticacao, periodicidade, frequenciaUso } = req.body;
  
  try {
    const [existing] = await pool.query('SELECT * FROM onpremise_to_cloud WHERE id = ?', [req.params.id]);
    if (existing.length === 0) {
      return res.status(404).json({ error: 'Registro não encontrado', code: 'NOT_FOUND' });
    }
    
    await pool.query(
      'UPDATE onpremise_to_cloud SET aplicacao_origem_id = ?, aplicacao_destino_id = ?, tipo_integracao_id = ?, tipo_autenticacao = ?, periodicidade = ?, frequencia_uso = ? WHERE id = ?',
      [aplicacaoOrigemId || existing[0].aplicacao_origem_id, aplicacaoDestinoId || existing[0].aplicacao_destino_id, tipoIntegracaoId || existing[0].tipo_integracao_id, tipoAutenticacao || existing[0].tipo_autenticacao, periodicidade || existing[0].periodicidade, frequenciaUso || existing[0].frequencia_uso, req.params.id]
    );
    
    const [updated] = await pool.query('SELECT * FROM onpremise_to_cloud WHERE id = ?', [req.params.id]);
    res.json({
      id: updated[0].id,
      integracaoId: updated[0].integracao_id,
      aplicacaoOrigemId: updated[0].aplicacao_origem_id,
    ...
    console.error('Erro ao atualizar onpremise-to-cloud:', error);
    res.status(500).json({ error: 'Erro ao atualizar dados', code: 'DATABASE_ERROR' });
  }
});

app.delete('/api/onpremise-to-cloud/:id', async (req, res) => {
  try {
    const [result] = await pool.query('DELETE FROM onpremise_to_cloud WHERE id = ?', [req.params.id]);
    if (result.affectedRows === 0) {
      return res.status(404).json({ error: 'Registro não encontrado', code: 'NOT_FOUND' });
    }
    res.status(204).send();
  } catch (error) {
    console.error('Erro ao excluir onpremise-to-cloud:', error);
    res.status(500).json({ error: 'Erro ao excluir dados', code: 'DATABASE_ERROR' });
  }
});

// OnPremise-to-OnPremise
app.get('/api/onpremise-to-onpremise', async (req, res) => {
  try {
    const integracaoId = req.query.integracaoId;
    const query = integracaoId 
      ? 'SELECT * FROM onpremise_to_onpremise WHERE integracao_id = ? ORDER BY created_at DESC'
      : 'SELECT * FROM onpremise_to_onpremise ORDER BY created_at DESC';
    const params = integracaoId ? [integracaoId] : [];
    const [rows] = await pool.query(query, params);
    res.json(rows.map(row => ({
      id: row.id,
    ...
    console.error('Erro ao listar onpremise-to-onpremise:', error);
    res.status(500).json({ error: 'Erro ao buscar dados', code: 'DATABASE_ERROR' });
  }
});

app.post('/api/onpremise-to-onpremise', async (req, res) => {
  const { integracaoId, aplicacaoOrigemId, aplicacaoDestinoId, tipoIntegracaoId, tipoAutenticacao, periodicidade, frequenciaUso } = req.body;
  ...
  try {
    const id = uuidv4();
    await pool.query(
      'INSERT INTO onpremise_to_onpremise (id, integracao_id, aplicacao_origem_id, aplicacao_destino_id, tipo_integracao_id, tipo_autenticacao, periodicidade, frequencia_uso) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
      [id, integracaoId, aplicacaoOrigemId, aplicacaoDestinoId, tipoIntegracaoId, tipoAutenticacao, periodicidade, frequenciaUso]
    );
    const [created] = await pool.query('SELECT * FROM onpremise_to_onpremise WHERE id = ?', [id]);
    res.status(201).json({
      id: created[0].id,
      integracaoId: created[0].integracao_id,
      aplicacaoOrigemId: created[0].aplicacao_origem_id,
    ...
    console.error('Erro ao criar onpremise-to-onpremise:', error);
    res.status(500).json({ error: 'Erro ao salvar dados', code: 'DATABASE_ERROR' });
  }
});

app.put('/api/onpremise-to-onpremise/:id', async (req, res) => {
  const { aplicacaoOrigemId, aplicacaoDestinoId, tipoIntegracaoId, tipoAutenticacao, periodicidade, frequenciaUso } = req.body;
  
  try {
    const [existing] = await pool.query('SELECT * FROM onpremise_to_onpremise WHERE id = ?', [req.params.id]);
    if (existing.length === 0) {
      return res.status(404).json({ error: 'Registro não encontrado', code: 'NOT_FOUND' });
    }
    
    await pool.query(
      'UPDATE onpremise_to_onpremise SET aplicacao_origem_id = ?, aplicacao_destino_id = ?, tipo_integracao_id = ?, tipo_autenticacao = ?, periodicidade = ?, frequencia_uso = ? WHERE id = ?',
      [aplicacaoOrigemId || existing[0].aplicacao_origem_id, aplicacaoDestinoId || existing[0].aplicacao_destino_id, tipoIntegracaoId || existing[0].tipo_integracao_id, tipoAutenticacao || existing[0].tipo_autenticacao, periodicidade || existing[0].periodicidade, frequenciaUso || existing[0].frequencia_uso, req.params.id]
    );
    
    const [updated] = await pool.query('SELECT * FROM onpremise_to_onpremise WHERE id = ?', [req.params.id]);
    res.json({
      id: updated[0].id,
      integracaoId: updated[0].integracao_id,
      aplicacaoOrigemId: updated[0].aplicacao_origem_id,
    ...
    console.error('Erro ao atualizar onpremise-to-onpremise:', error);
    res.status(500).json({ error: 'Erro ao atualizar dados', code: 'DATABASE_ERROR' });
  }
});

app.delete('/api/onpremise-to-onpremise/:id', async (req, res) => {
  try {
    const [result] = await pool.query('DELETE FROM onpremise_to_onpremise WHERE id = ?', [req.params.id]);
    if (result.affectedRows === 0) {
      return res.status(404).json({ error: 'Registro não encontrado', code: 'NOT_FOUND' });
    }
    res.status(204).send();
  } catch (error) {
    console.error('Erro ao excluir onpremise-to-onpremise:', error);
    res.status(500).json({ error: 'Erro ao excluir dados', code: 'DATABASE_ERROR' });
  }
});
  ...
  try {
    ...
    await pool.query(
      `INSERT INTO estruturas_projeto (
        ...
        repositorios, pat_token, estruturas_geradas
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        id,
        estrutura.produto,
...
// ==================== AZURE DEVOPS PROJETOS (CRUD) ====================

// GET /api/azure-devops-projetos - Listar todos os projetos Azure DevOps
app.get('/api/azure-devops-projetos', async (req, res) => {
  try {
    const [rows] = await pool.query(`
      SELECT 
        id, produto, projeto, work_item_process as workItemProcess,
        team_name as teamName, data_inicial as dataInicial,
        criar_time_sustentacao as criarTimeSustentacao,
        azure_project_id as azureProjectId, azure_project_url as azureProjectUrl,
        teams_created as teamsCreated, iterations_count as iterationsCount,
        areas_count as areasCount, status, error_message as errorMessage,
        data_criacao as dataCriacao, data_atualizacao as dataAtualizacao
      FROM azure_devops_projetos 
      ORDER BY data_criacao DESC
    `);
    
    const projetos = rows.map(row => ({
      ...row,
      teamsCreated: typeof row.teamsCreated === 'string' ? JSON.parse(row.teamsCreated) : row.teamsCreated,
      criarTimeSustentacao: Boolean(row.criarTimeSustentacao)
    }));
    
    console.log(`[API] GET /api/azure-devops-projetos - Retornando ${projetos.length} projetos`);
    res.json(projetos);
  } catch (error) {
    console.error('Erro ao buscar projetos Azure DevOps:', error);
    res.status(500).json({ error: 'Erro ao buscar dados', code: 'DATABASE_ERROR' });
  }
});

// GET /api/azure-devops-projetos/:id - Buscar projeto por ID
app.get('/api/azure-devops-projetos/:id', async (req, res) => {
  try {
    const [rows] = await pool.query(`
      SELECT 
        id, produto, projeto, work_item_process as workItemProcess,
    ...
    if (rows.length === 0) {
      return res.status(404).json({ error: 'Projeto não encontrado', code: 'NOT_FOUND' });
    }
    
    const projeto = {
      ...rows[0],
      teamsCreated: typeof rows[0].teamsCreated === 'string' ? JSON.parse(rows[0].teamsCreated) : rows[0].teamsCreated,
      criarTimeSustentacao: Boolean(rows[0].criarTimeSustentacao)
    };
    
    res.json(projeto);
  } catch (error) {
    console.error('Erro ao buscar projeto Azure DevOps:', error);
    res.status(500).json({ error: 'Erro ao buscar dados', code: 'DATABASE_ERROR' });
  }
});

// POST /api/azure-devops-projetos - Criar novo registro de projeto
app.post('/api/azure-devops-projetos', async (req, res) => {
  try {
    const {
      id, produto, projeto, workItemProcess, teamName, dataInicial,
      criarTimeSustentacao, azureProjectId, azureProjectUrl, teamsCreated,
      iterationsCount, areasCount, status, errorMessage
    } = req.body;
    
    console.log(`[API] POST /api/azure-devops-projetos - Criando projeto: ${produto} ${projeto}`);
    
    await pool.query(`
      INSERT INTO azure_devops_projetos (
        id, produto, projeto, work_item_process, team_name, data_inicial,
        criar_time_sustentacao, azure_project_id, azure_project_url,
        teams_created, iterations_count, areas_count, status, error_message
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `, [
      id || uuidv4(),
      produto,
      ...
      workItemProcess || 'Scrum',
      teamName,
      formatDateForMySQL(dataInicial),
      criarTimeSustentacao || false,
      azureProjectId || null,
      azureProjectUrl || null,
      teamsCreated ? JSON.stringify(teamsCreated) : null,
      iterationsCount || 0,
      areasCount || 0,
      status || 'pendente',
      errorMessage || null
    ...
    res.status(201).json({ message: 'Projeto criado com sucesso', id: id || uuidv4() });
  } catch (error) {
    console.error('Erro ao criar projeto Azure DevOps:', error);
    res.status(500).json({ error: 'Erro ao criar projeto', code: 'DATABASE_ERROR' });
  }
});

// PUT /api/azure-devops-projetos/:id - Atualizar projeto
app.put('/api/azure-devops-projetos/:id', async (req, res) => {
  try {
    const {
      produto, projeto, workItemProcess, teamName, dataInicial,
      criarTimeSustentacao, azureProjectId, azureProjectUrl, teamsCreated,
      iterationsCount, areasCount, status, errorMessage
    } = req.body;
    
    console.log(`[API] PUT /api/azure-devops-projetos/${req.params.id}`);
    
    const [result] = await pool.query(`
      UPDATE azure_devops_projetos SET
        produto = ?, projeto = ?, work_item_process = ?, team_name = ?,
        data_inicial = ?, criar_time_sustentacao = ?, azure_project_id = ?,
        azure_project_url = ?, teams_created = ?, iterations_count = ?,
        areas_count = ?, status = ?, error_message = ?
      WHERE id = ?
    `, [
      produto,
      projeto,
    ...
    console.error('Erro ao atualizar projeto Azure DevOps:', error);
    res.status(500).json({ error: 'Erro ao atualizar projeto', code: 'DATABASE_ERROR' });
  }
});

// DELETE /api/azure-devops-projetos/:id - Deletar projeto
app.delete('/api/azure-devops-projetos/:id', async (req, res) => {
  try {
    const [result] = await pool.query('DELETE FROM azure_devops_projetos WHERE id = ?', [req.params.id]);
    ...
    console.error('Erro ao deletar projeto Azure DevOps:', error);
    res.status(500).json({ error: 'Erro ao deletar projeto', code: 'DATABASE_ERROR' });
  }
});
