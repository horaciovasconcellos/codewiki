...
# Script para carga em lote de Tecnologias
# Uso: ./load-tecnologias.sh <arquivo.json>
# Exemplo: ./load-tecnologias.sh ../data-templates/tecnologias-carga.json
...
if [ -z "$INPUT_FILE" ]; then
    ...
    echo ""
    echo "Exemplos:"
    echo "  $0 tecnologias.json"
    echo "  $0 ../data-templates/tecnologias-carga.json"
...
echo -e "${BLUE}📊 Total de tecnologias a processar:${NC} $TOTAL"
echo ""
...
LOG_FILE="load-tecnologias-$(date +%Y%m%d_%H%M%S).log"
echo "Iniciando carga em $(date)" > "$LOG_FILE"

# Ler o array JSON e processar cada item
jq -c '.[]' "$INPUT_FILE" | while IFS= read -r tecnologia; do
    SIGLA=$(echo "$tecnologia" | jq -r '.sigla // "SEM-SIGLA"')
    NOME=$(echo "$tecnologia" | jq -r '.nome')
    ...
    if [ "$SIGLA" != "SEM-SIGLA" ] && [ "$SIGLA" != "null" ]; then
        EXISTING=$(curl -s "$API_URL/api/tecnologias" | jq -r --arg sigla "$SIGLA" '.[] | select(.sigla == $sigla) | .id')
    fi
    
    if [ -n "$EXISTING" ] && [ "$EXISTING" != "null" ]; then
        # Atualizar registro existente
        RESPONSE=$(curl -s -X PUT "$API_URL/api/tecnologias/$EXISTING" \
        ...
        # Criar novo registro
        RESPONSE=$(curl -s -X POST "$API_URL/api/tecnologias" \
            -H "Content-Type: application/json" \
            -d "$tecnologia")
...
TOTAL_DB=$(curl -s "$API_URL/api/tecnologias" | jq 'length')
echo -e "${GREEN}✓ Total de tecnologias no banco:${NC} $TOTAL_DB"
echo ""
...
echo ""
echo -e "${BLUE}=========================================="
echo "TECNOLOGIAS CADASTRADAS"
echo -e "==========================================${NC}"
curl -s "$API_URL/api/tecnologias" | jq -r '.[] | "\(.sigla // "N/A") - \(.nome) (\(.categoria // "N/A"), \(.status // "N/A"))"'
