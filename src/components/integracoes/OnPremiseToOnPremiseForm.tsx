...
import { OnPremiseToOnPremise, Integracao, Comunicacao, Aplicacao, TipoAutenticacao, Periodicidade, FrequenciaUso } from '@/lib/types';
import { Button } from '@/components/ui/button';
import { Label } from '@/components/ui/label';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select';

interface OnPremiseToOnPremiseFormProps {
  data?: OnPremiseToOnPremise;
  integracaoAtual?: Integracao;
  integracoes?: Integracao[];
  aplicacoes: Aplicacao[];
  comunicacoes: Comunicacao[];
  onSave: (data: OnPremiseToOnPremise) => void;
  onCancel: () => void;
}
...
export function OnPremiseToOnPremiseForm({ data, integracaoAtual, integracoes, aplicacoes, comunicacoes, onSave, onCancel }: OnPremiseToOnPremiseFormProps) {
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="grid gap-6 md:grid-cols-2">
        <div className="grid gap-2">
          <Label htmlFor="integracaoId">Integração <span className="text-red-500">*</span></Label>
          <Select value={formData.integracaoId} onValueChange={(v) => setFormData({ ...formData, integracaoId: v })} required>
            <SelectTrigger>
              <SelectValue placeholder="Selecione" />
            </SelectTrigger>
            <SelectContent>
              {integracoesDisponiveis.map((int) => (
                <SelectItem key={int.id} value={int.id}>{int.sigla} - {int.nome}</SelectItem>
              ))}
            </SelectContent>
          </Select>
        </div>
